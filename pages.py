# pages.py - پنل عقاب نسخه حرفه‌ای
...
.logo-icon{width:40px;height:40px;border-radius:12px;background:linear-gradient(135deg,#FF6B35,#FF4500);display:flex;align-items:center;justify-content:center;font-size:20px;flex-shrink:0}
.logo-name{font-size:14px;font-weight:800;background:linear-gradient(135deg,#FF8C00,#FF4500);-webkit-background-clip:text;-webkit-text-fill-color:transparent}
.logo-sub{font-size:8px;color:var(--t3);margin-top:1px}
.nav-wrap{flex:1;overflow-y:auto;padding:8px 0}
.nav-it{display:flex;align-items:center;gap:10px;padding:10px 14px;color:var(--t3);font-size:12px;cursor:pointer;border-right:2px solid transparent;transition:all .2s;margin:2px 6px;border-radius:8px}
.nav-it i{font-size:15px;width:20px;text-align:center;flex-shrink:0}
...
.nav-it .badge{margin-right:auto;background:var(--accent);color:#fff;font-size:8px;padding:1px 8px;border-radius:12px;font-weight:700}
.sb-foot{padding:12px 14px;border-top:1px solid var(--card-b)}
.logout-btn{display:flex;align-items:center;justify-content:center;gap:7px;background:var(--red-bg);color:var(--red-t);border-radius:8px;padding:7px;font-size:11px;font-weight:500;font-family:inherit;border:1px solid rgba(239,68,68,0.15);cursor:pointer;width:100%;transition:.2s}
.logout-btn:hover{background:rgba(239,68,68,0.2)}
.mob-top{display:none;position:fixed;top:0;right:0;left:0;height:50px;background:var(--card);backdrop-filter:blur(30px);border-bottom:1px solid var(--card-b);z-index:150;align-items:center;justify-content:space-between;padding:0 12px}
.mob-top .ml{display:flex;align-items:center;gap:8px}
.mob-logo{width:28px;height:28px;border-radius:8px;background:linear-gradient(135deg,#FF6B35,#FF4500);display:flex;align-items:center;justify-content:center;font-size:14px}
.mob-title{color:var(--t1);font-size:12px;font-weight:700}
.menu-btn{background:var(--accent-d);border:1px solid var(--card-b);color:var(--t2);width:32px;height:32px;border-radius:8px;font-size:16px;display:flex;align-items:center;justify-content:center;cursor:pointer}
.overlay{display:none;position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:190;backdrop-filter:blur(4px)}
.overlay.show{display:block}
.main{margin-right:var(--sidebar-w);flex:1;padding:20px 24px 30px;min-width:0;transition:margin .3s}
...
.badge{font-size:9px;padding:3px 10px;border-radius:16px;font-weight:700;display:inline-flex;align-items:center;gap:4px;white-space:nowrap}
...
.stat-card{background:var(--card);backdrop-filter:blur(20px);border:1px solid var(--card-b);border-radius:var(--radius);padding:14px 12px;transition:all .3s;text-align:center}
.stat-card:hover{border-color:var(--card-bh);transform:translateY(-2px)}
.stat-card .icon{font-size:22px;margin-bottom:4px;display:block}
.stat-card .number{font-size:22px;font-weight:800;color:var(--t1);line-height:1.2}
.stat-card .number.small{font-size:16px}
.stat-card .label{font-size:10px;color:var(--t3);margin-top:3px;font-weight:500}
.stat-card .sub{font-size:8px;color:var(--t3);margin-top:1px;opacity:.6}

/* ===== اینباند ===== */
.inbound-card{background:var(--card);backdrop-filter:blur(20px);border:1px solid var(--card-b);border-radius:var(--radius);padding:18px 20px;margin-bottom:16px}
.inbound-card .title{font-size:14px;font-weight:700;color:var(--t1);display:flex;align-items:center;gap:8px;margin-bottom:12px}
.inbound-card .title i{color:var(--accent)}
.inbound-info{display:grid;grid-template-columns:repeat(auto-fill,minmax(150px,1fr));gap:10px;margin-bottom:12px}
.inbound-info .item{background:rgba(255,255,255,0.02);border-radius:8px;padding:8px 12px;text-align:center}
.inbound-info .item .val{font-size:14px;font-weight:700;color:var(--t1)}
.inbound-info .item .lbl{font-size:9px;color:var(--t3);margin-top:2px}
.inbound-actions{display:flex;gap:8px;flex-wrap:wrap;margin-top:8px}
.inbound-actions .btn{font-size:11px;padding:6px 14px}

/* ===== کاربران ===== */
.user-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(320px,1fr));gap:14px}
.user-card{background:var(--card);backdrop-filter:blur(20px);border:1px solid var(--card-b);border-radius:var(--radius);padding:14px 16px;transition:all .3s}
.user-card:hover{border-color:var(--card-bh);transform:translateY(-2px)}
.user-card .head{display:flex;align-items:center;justify-content:space-between;margin-bottom:4px}
.user-card .name{font-size:13px;font-weight:700;color:var(--t1);display:flex;align-items:center;gap:5px}
.user-card .status{font-size:9px;font-weight:700;padding:2px 10px;border-radius:10px}
.user-card .status.on{background:var(--green-bg);color:var(--green-t)}
.user-card .status.off{background:var(--red-bg);color:var(--red-t)}
.user-card .uuid{font-family:monospace;font-size:8px;color:var(--t3);margin-bottom:6px;word-break:break-all}
.user-card .info{display:grid;grid-template-columns:1fr 1fr;gap:3px 10px;font-size:10px;color:var(--t2);margin-bottom:4px}
.user-card .quota-info{display:flex;justify-content:space-between;font-size:10px;color:var(--t2);margin-bottom:3px}
.user-card .quota-bar{height:4px;border-radius:3px;background:var(--accent-d);overflow:hidden;margin-bottom:8px}
.user-card .quota-fill{height:100%;border-radius:3px;background:linear-gradient(90deg,#FF6B35,#FF4500);transition:width .6s ease}
.user-card .last-seen{font-size:9px;color:var(--t3);margin-bottom:6px}
.user-card .actions{display:flex;gap:4px;flex-wrap:wrap}
.user-card .actions .btn{font-size:9px;padding:4px 10px;border-radius:6px;flex:1;justify-content:center}
.user-card .lock-badge{font-size:8px;color:var(--amber-t);background:var(--amber-bg);padding:1px 6px;border-radius:6px}

/* ===== دکمه‌ها ===== */
.btn{font-family:inherit;font-size:11px;font-weight:600;border-radius:8px;padding:6px 14px;cursor:pointer;display:inline-flex;align-items:center;gap:5px;border:none;transition:all .2s;white-space:nowrap}
...
.modal{background:var(--card);backdrop-filter:blur(30px);border:1px solid var(--card-b);border-radius:16px;padding:24px 22px;max-width:480px;width:calc(100% - 24px);max-height:90vh;overflow-y:auto;position:relative;animation:pageIn .3s ease;box-shadow:var(--shadow)}
.modal-close{position:absolute;top:12px;left:12px;background:rgba(255,255,255,0.03);border:1px solid var(--card-b);color:var(--t2);width:28px;height:28px;border-radius:8px;font-size:14px;display:flex;align-items:center;justify-content:center;cursor:pointer;border:none;transition:.2s}
.modal-close:hover{background:var(--red-bg);color:var(--red-t)}
.modal-title{font-size:15px;font-weight:700;color:var(--t1);margin-bottom:16px;display:flex;align-items:center;gap:8px}
.modal-title i{color:var(--accent);font-size:17px}
.fg{display:flex;flex-direction:column;gap:3px;margin-bottom:10px}
.fg label{font-size:9px;color:var(--t3);font-weight:700;text-transform:uppercase;letter-spacing:.05em;display:flex;align-items:center;gap:4px}
.fi{width:100%;padding:8px 12px;border-radius:8px;border:1px solid var(--card-b);background:rgba(0,0,0,.2);color:var(--t1);font-family:inherit;font-size:11px;outline:none;transition:.2s}
.fi:focus{border-color:var(--accent);box-shadow:0 0 0 3px rgba(255,80,20,.06)}
.fi::placeholder{color:var(--t3)}
select.fi{appearance:none;cursor:pointer}

/* ===== اتصالات ===== */
.conn-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(240px,1fr));gap:10px}
.conn-card{background:var(--card);backdrop-filter:blur(20px);border:1px solid var(--card-b);border-radius:12px;padding:12px 14px;transition:.2s}
.conn-card:hover{border-color:var(--card-bh)}
.conn-card .ip{font-family:monospace;font-size:12px;font-weight:700;color:var(--t1);display:flex;align-items:center;gap:5px}
.conn-card .label{font-size:9px;color:var(--t3);margin-top:2px}
...
.conn-status-dot{display:inline-block;width:6px;height:6px;border-radius:50%;background:#34D399;animation:pulse 1.5s infinite;margin-left:3px}

/* ===== تنظیمات ===== */
.settings-card{background:var(--card);backdrop-filter:blur(20px);border:1px solid var(--card-b);border-radius:var(--radius);padding:18px 20px;max-width:500px;margin-bottom:14px}
.settings-card .title{font-size:14px;font-weight:700;color:var(--t1);margin-bottom:12px;display:flex;align-items:center;gap:8px}
.settings-card .title i{color:var(--accent)}
.settings-card .field{margin-bottom:10px}
.settings-card .field label{font-size:10px;color:var(--t3);display:block;margin-bottom:3px;font-weight:600}
.settings-card .field input{width:100%;padding:8px 12px;border-radius:8px;border:1px solid var(--card-b);background:rgba(0,0,0,.2);color:var(--t1);font-family:inherit;font-size:12px;outline:none;transition:.2s}
.settings-card .field input:focus{border-color:var(--accent);box-shadow:0 0 0 3px rgba(255,80,20,.06)}
.settings-card .btn{width:100%;justify-content:center;margin-top:4px;font-size:12px;padding:8px}
.settings-card .toggle-row{display:flex;align-items:center;justify-content:space-between;padding:10px 0;border-bottom:1px solid var(--card-b)}
.settings-card .toggle-row .toggle-label{font-size:12px;color:var(--t2);display:flex;align-items:center;gap:6px}
.settings-card .toggle-row .toggle-label i{font-size:16px}
.switch{position:relative;width:40px;height:22px;background:var(--t3);border-radius:11px;cursor:pointer;transition:.3s;flex-shrink:0}
.switch.on{background:linear-gradient(135deg,#FF6B35,#FF4500)}
.switch .slider{position:absolute;top:2px;right:2px;width:18px;height:18px;background:#fff;border-radius:50%;transition:.3s;box-shadow:0 2px 6px rgba(0,0,0,0.2)}
.switch.on .slider{right:20px}

.toast{position:fixed;bottom:20px;left:50%;transform:translateX(-50%) translateY(50px);background:var(--card);backdrop-filter:blur(30px);border:1px solid var(--card-b);color:var(--t1);border-radius:10px;padding:10px 20px;font-size:12px;opacity:0;transition:all .3s;z-index:999;pointer-events:none;box-shadow:var(--shadow);display:flex;align-items:center;gap:6px}
...
@media(max-width:768px){
  ...
  .main{margin-right:0;padding-top:60px;padding:60px 12px 20px}
  .mob-top{display:flex}
  .user-grid{grid-template-columns:1fr}
  .stats-grid{grid-template-columns:1fr 1fr}
  .inbound-info{grid-template-columns:1fr 1fr}
}
@media(max-width:480px){.stats-grid{grid-template-columns:1fr}.main{padding:56px 8px 16px}}
</style>
</head>
<body>
...
<div class="modal-bg" id="modal-user">
  <div class="modal">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px">
      ...
      <div class="fg"><label><i class="ti ti-devices"></i> دستگاه</label><input class="fi" id="user-devices" type="number" min="0" max="10" value="1" placeholder="0=نامحدود"></div>
    </div>
    <div class="fg"><label><i class="ti ti-lock"></i> رمز کانفیگ (اختیاری)</label><input class="fi" id="user-password" type="password" placeholder="برای حذف/ویرایش" dir="ltr"></div>
...
<div class="modal-bg" id="modal-edit">
  <div class="modal">
    ...
    <div class="fg" id="edit-password-section"><label><i class="ti ti-lock"></i> رمز کانفیگ</label><input class="fi" id="edit-password" type="password" placeholder="رمز کانفیگ را وارد کنید" dir="ltr"></div>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
      <div class="fg"><label><i class="ti ti-devices"></i> دستگاه</label><input class="fi" id="edit-devices" type="number" min="0" max="10" placeholder="0=نامحدود"></div>
...
<div class="modal-bg" id="modal-delete">
  <div class="modal" style="max-width:360px">
    ...
    <p style="font-size:12px;color:var(--t2);margin-bottom:12px">برای حذف، رمز کانفیگ را وارد کنید.</p>
    <div class="fg"><label><i class="ti ti-lock"></i> رمز کانفیگ</label><input class="fi" id="delete-password" type="password" placeholder="رمز کانفیگ" dir="ltr"></div>
...
<aside class="sidebar" id="sb">
  ...
  <div class="nav-wrap">
    <div class="nav-it on" data-pg="dashboard"><i class="ti ti-layout-dashboard"></i> داشبورد</div>
...
<section class="pg on" id="pg-dashboard">
  <div class="topbar">
    <div><div class="tb-title"><i class="ti ti-layout-dashboard"></i> داشبورد</div><div class="tb-sub" id="last-update">بروزرسانی: لحظه‌ای</div></div>
    <div class="tb-right">
      <span class="badge bg-fire" id="online-badge"><span class="dot dg"></span> ۰ آنلاین</span>
      <button class="btn btn-p btn-sm" onclick="openModal('modal-user')"><i class="ti ti-plus"></i> کاربر جدید</button>
    </div>
  </div>

  <div class="stats-grid">
    <div class="stat-card"><span class="icon">📊</span><div class="number" id="stat-traffic">۰</div><div class="label">ترافیک</div><div class="sub">مگابایت</div></div>
    ...
    <div class="stat-card"><span class="icon">📶</span><div class="number small" id="stat-speed">۰ B/s</div><div class="label">سرعت دانلود</div><div class="sub">لحظه‌ای</div></div>
  ...
  <!-- کاربران اخیر -->
  <div style="background:var(--card);backdrop-filter:blur(20px);border:1px solid var(--card-b);border-radius:var(--radius);padding:14px 16px;margin-top:6px">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">
      <span style="font-size:13px;font-weight:700;color:var(--t1)">👥 کاربران اخیر</span>
      <button class="btn btn-sm btn-o" onclick="loadDashboard()"><i class="ti ti-refresh"></i> بروزرسانی</button>
    </div>
    <div id="recent-users" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:8px">
      <div class="empty" style="padding:20px"><i class="ti ti-users"></i><p style="font-size:11px">هیچ کاربری ساخته نشده</p></div>
    </div>
  </div>
</section>
...
<section class="pg" id="pg-inbound">
  <div class="topbar"><div><div class="tb-title"><i class="ti ti-plug"></i> اینباند</div><div class="tb-sub">تنظیمات ورودی و ساخت کاربر</div></div></div>
  
  <div class="inbound-card">
    <div class="title"><i class="ti ti-info-circle"></i> وضعیت اینباند</div>
    <div class="inbound-info">
      <div class="item"><div class="val" id="inbound-port">۴۴۳</div><div class="lbl">پورت</div></div>
      <div class="item"><div class="val" id="inbound-protocol">VLESS</div><div class="lbl">پروتکل</div></div>
      <div class="item"><div class="val" id="inbound-host">—</div><div class="lbl">هاست</div></div>
      <div class="item"><div class="val" style="color:#34D399">✅ فعال</div><div class="lbl">وضعیت</div></div>
    </div>
    <div class="inbound-actions">
      <button class="btn btn-p" onclick="openModal('modal-user')"><i class="ti ti-user-plus"></i> ساخت کاربر</button>
      <button class="btn btn-o" onclick="openModal('modal-inbound-settings')"><i class="ti ti-settings"></i> تنظیمات</button>
    </div>
  </div>

  <!-- کاربران اینباند -->
  <div style="background:var(--card);backdrop-filter:blur(20px);border:1px solid var(--card-b);border-radius:var(--radius);padding:14px 16px">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">
      <span style="font-size:13px;font-weight:700;color:var(--t1)">📋 کاربران اینباند</span>
      <button class="btn btn-sm btn-o" onclick="loadUsers()"><i class="ti ti-refresh"></i></button>
    </div>
    <div id="inbound-users-grid" class="user-grid">
      <div class="empty"><i class="ti ti-users"></i><p style="font-size:11px">هیچ کاربری ساخته نشده</p></div>
    </div>
  </div>
</section>
...
<section class="pg" id="pg-users">
  <div class="topbar">
    <div><div class="tb-title"><i class="ti ti-users"></i> کاربران</div><div class="tb-sub" id="users-count">۰ کاربر</div></div>
    <div class="tb-right">
      <button class="btn btn-p btn-sm" onclick="openModal('modal-user')"><i class="ti ti-plus"></i> کاربر جدید</button>
    </div>
  </div>
...
<section class="pg" id="pg-connections">
  <div class="topbar"><div><div class="tb-title">🔌 اتصالات زنده</div><div class="tb-sub" id="conn-count">۰ اتصال</div></div><div class="tb-right"><span class="badge bg-green"><span class="dot dg pulse"></span> فعال</span><button class="btn btn-sm btn-o" onclick="loadConnections()"><i class="ti ti-refresh"></i></button></div></div>
...
<section class="pg" id="pg-settings">
  ...
  <div class="settings-card">
    <div class="title"><i class="ti ti-key"></i> 🔑 تغییر رمز پنل</div>
    <div class="field"><label>رمز فعلی</label><input class="fi" id="old-password" type="password" placeholder="رمز فعلی" dir="ltr"></div>
    <div class="field"><label>رمز جدید</label><input class="fi" id="new-password" type="password" placeholder="رمز جدید (حداقل ۴ کاراکتر)" dir="ltr"></div>
    <div class="field"><label>تکرار رمز جدید</label><input class="fi" id="confirm-password" type="password" placeholder="تکرار رمز جدید" dir="ltr"></div>
    <button class="btn btn-p" onclick="changePassword()"><i class="ti ti-key"></i> تغییر رمز</button>
    <div id="password-result" style="margin-top:10px;display:none;font-size:12px;"></div>
  </div>

  <div class="settings-card">
    <div class="title"><i class="ti ti-plug"></i> تنظیمات اینباند</div>
    <div class="field"><label>پورت</label><input class="fi" id="inbound-port-setting" type="number" min="1" max="65535" value="443"></div>
    <button class="btn btn-p" onclick="updateInbound()"><i class="ti ti-check"></i> ذخیره اینباند</button>
  </div>
</section>
...
<section class="pg" id="pg-logs">
  ...
  <div style="background:var(--card);backdrop-filter:blur(20px);border:1px solid var(--card-b);border-radius:var(--radius);padding:12px 14px;max-height:500px;overflow-y:auto">
    <div id="logs-container" style="font-family:monospace;font-size:10px;color:var(--t2);direction:ltr;text-align:left;line-height:1.6">
      <div class="empty"><i class="ti ti-notes"></i><p>هیچ لاگی ثبت نشده</p></div>
    </div>
  </div>
</section>
...
<section class="pg" id="pg-backup">
  <div class="topbar"><div><div class="tb-title"><i class="ti ti-database"></i> بکاپ</div><div class="tb-sub">ذخیره و بازیابی اطلاعات</div></div></div>
  <div class="settings-card">
    <div class="title"><i class="ti ti-download"></i> بکاپ‌گیری</div>
    <p style="font-size:11px;color:var(--t2);margin-bottom:12px;line-height:1.6">از اطلاعات کاربران و تنظیمات بکاپ بگیرید.</p>
...
async function loadDashboard() {
  try {
    ...
    if (!recent.length) {
      grid.innerHTML = '<div class="empty" style="padding:20px"><i class="ti ti-users"></i><p style="font-size:11px">هیچ کاربری ساخته نشده</p></div>';
    } else {
      grid.innerHTML = recent.map(l => `
        <div style="background:rgba(255,255,255,0.02);border-radius:8px;padding:8px 10px;display:flex;justify-content:space-between;align-items:center">
          <div>
            <div style="font-size:11px;font-weight:600;color:var(--t1)">${esc(l.label)}</div>
            <div style="font-size:8px;color:var(--t3)">${l.active ? '🟢 فعال' : '🔴 غیرفعال'}</div>
          </div>
          <div style="font-size:9px;color:var(--t2)">${fmtB(l.used_bytes || 0)}</div>
        </div>
      `).join('');
    }
...
async function updateInbound() {
  ...
  try {
    const r = await authF('/api/inbound', {
      ...
      body: JSON.stringify({ port: port, protocol: 'vless' })
    });
    if (!r.ok) { toast('❌ خطا', 'err'); return; }
    toast('✅ اینباند بروزرسانی شد', 'ok');
    loadInbound();
...
async function loadUsers() {
  try {
    ...
    if (!links.length) {
      const empty = '<div class="empty"><i class="ti ti-users"></i><p>هیچ کاربری ساخته نشده</p></div>';
    ...
    const html = links.map(l => {
      ...
      const subLink = `https://${window.location.host}/sub/${l.uuid}`;
      
      return `<div class="user-card">
        <div class="head">
          <div class="name">🦅 ${esc(l.label)} ${l.has_password ? '<span class="lock-badge"><i class="ti ti-lock"></i></span>' : ''}</div>
        ...
        <div class="info">
          ...
          <span>📅 ${l.expired ? '⛔ منقضی' : '✅ معتبر'}</span>
        </div>
        ...
        <div class="actions">
          ...
          <button class="btn btn-pur btn-sm" onclick="navigator.clipboard.writeText('${esc(subLink)}').then(()=>toast('✅ کپی شد','ok'))"><i class="ti ti-link"></i></button>
...
async function saveEdit() {
  ...
  try {
    ...
    if (!r.ok) {
      if (r.status === 403) { toast('❌ رمز اشتباه است', 'err'); return; }
      throw new Error();
    }
...
async function loadLogs() {
  try {
    const r = await authF('/api/activity?limit=100');
    const data = await r.json();
    const logs = data.logs || [];
    document.getElementById('logs-count').textContent = logs.length + ' لاگ';
    const container = document.getElementById('logs-container');
    if (!logs.length) {
      container.innerHTML = '<div class="empty"><i class="ti ti-notes"></i><p>هیچ لاگی ثبت نشده</p></div>';
      return;
    }
    container.innerHTML = logs.map(log => {
      const time = log.time ? new Date(log.time).toLocaleString('fa-IR') : '—';
      const levelColor = log.level === 'err' ? '#F87171' : log.level === 'warn' ? '#FCD34D' : '#34D399';
      return `<div style="padding:4px 0;border-bottom:1px solid rgba(255,255,255,0.02);display:flex;gap:8px">
        <span style="color:${levelColor};font-weight:700">[${(log.level || 'info').toUpperCase()}]</span>
...
async function changePassword() {
  ...
  if (!oldPw || !newPw || !confirmPw) {
    ...
    result.innerHTML = '❌ همه فیلدها را پر کنید';
    return;
  }
  if (newPw.length < 4) {
    result.style.display = 'block';
    result.style.color = '#F87171';
    result.innerHTML = '❌ رمز جدید حداقل ۴ کاراکتر باشد';
    return;
  }
  if (newPw !== confirmPw) {
    result.style.display = 'block';
    result.style.color = '#F87171';
    result.innerHTML = '❌ رمزها مطابقت ندارند';
    return;
  }
  ...
  try {
    ...
    if (!r.ok) {
      result.style.display = 'block';
      result.style.color = '#F87171';
      result.innerHTML = '❌ ' + (data.detail || data.message || 'خطا');
      return;
    }
    result.style.display = 'block';
    result.style.color = '#34D399';
    result.innerHTML = '✅ رمز با موفقیت تغییر کرد!';
    ...
    result.style.display = 'block';
    result.style.color = '#F87171';
    result.innerHTML = '❌ خطا: ' + e.message;
  }
}
...
def get_sub_page_html(uuid: str, link: dict) -> str:
    ...
    new_vless_link = generate_vless_link(
        uuid, host, remark=remark,
        protocol=protocol, fingerprint=fingerprint, port=port
    )
    
    conns_html = ""
    if active_connections > 0:
        conns_html = f"""
        <div style="background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.04);border-radius:10px;padding:10px 12px;margin:10px 0">
            <div style="display:flex;align-items:center;gap:5px;margin-bottom:6px;font-size:10px;color:#8A4A3A">
                <span style="display:inline-block;width:6px;height:6px;border-radius:50%;background:#34D399;animation:pulse 1.5s infinite"></span>
        ...
        for conn in active_connections_list[:10]:
            ...
            conns_html += f"""
                <span style="font-family:monospace;font-size:9px;background:rgba(255,80,20,0.06);border:1px solid rgba(255,80,20,0.06);padding:2px 8px;border-radius:4px;color:#8A4A3A">🔵 {ip}</span>
            """
        if len(active_connections_list) > 10:
            conns_html += f"""
                <span style="font-family:monospace;font-size:9px;background:rgba(255,80,20,0.04);padding:2px 8px;border-radius:4px;color:#5A3A2A">+{len(active_connections_list)-10} بیشتر</span>
            """
        conns_html += """
            </div>
        </div>
        ...
        conns_html = f"""
        <div style="background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.04);border-radius:10px;padding:8px 12px;margin:10px 0;text-align:center">
            <span style="font-size:10px;color:#5A3A2A">🔴 بدون اتصال فعال</span>
        </div>
        """
...
@keyframes flameFlicker{{0%{{opacity:0.6;transform:scale(1)}}50%{{opacity:1;transform:scale(1.02)}}100%{{opacity:0.6;transform:scale(1)}}}}
@keyframes pulse{{0%,100%{{opacity:1}}50%{{opacity:.25}}}}
body{{font-family:'Vazirmatn',sans-serif;min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px;color:#F0EEFF;background:linear-gradient(135deg,#0a0a0f,#1a0a0a,#0a0a1a);background-size:400% 400%;animation:fireBG 8s ease infinite;}}
.fire-particles{{position:fixed;inset:0;z-index:0;pointer-events:none;overflow:hidden;}}
.fire-particle{{position:absolute;border-radius:50%;background:radial-gradient(circle,rgba(255,120,50,0.3),rgba(255,50,0,0));width:6px;height:6px;animation:floatFire 12s ease-in-out infinite;}}
@keyframes floatFire{{0%{{transform:translateY(100vh) scale(0) rotate(0deg);opacity:0}}20%{{opacity:1}}80%{{opacity:1}}100%{{transform:translateY(-10vh) scale(1.5) rotate(720deg);opacity:0}}}}
...
.glow1{{width:400px;height:400px;background:rgba(255,80,20,0.05);top:-150px;right:-80px}}
.glow2{{width:300px;height:300px;background:rgba(255,150,50,0.04);bottom:-80px;left:-60px;animation-delay:2s}}
.card{{position:relative;z-index:10;background:rgba(15,15,30,0.8);backdrop-filter:blur(30px);border:1px solid rgba(255,255,255,0.05);border-radius:24px;padding:32px 30px 28px;max-width:460px;width:100%;box-shadow:0 0 80px rgba(0,0,0,0.5);animation:cardIn 0.6s ease;}}
@keyframes cardIn{{from{{opacity:0;transform:translateY(30px) scale(0.96)}}to{{opacity:1;transform:translateY(0) scale(1)}}}}
.brand{{display:flex;align-items:center;gap:12px;margin-bottom:22px;padding-bottom:14px;border-bottom:1px solid rgba(255,255,255,0.04);}}
.brand-icon{{width:44px;height:44px;border-radius:14px;background:linear-gradient(135deg,#FF6B35,#FF4500,#FF8C00);display:flex;align-items:center;justify-content:center;font-size:24px;flex-shrink:0;box-shadow:0 0 40px rgba(255,80,20,0.15);}}
.brand-text .name{{font-size:16px;font-weight:800;background:linear-gradient(135deg,#FF8C00,#FF4500,#FF6B35);-webkit-background-clip:text;-webkit-text-fill-color:transparent;}}
.brand-text .sub{{font-size:9px;color:#8A4A3A;margin-top:1px}}
.user-header{{display:flex;align-items:center;justify-content:space-between;margin-bottom:4px}}
.user-name{{font-size:20px;font-weight:800;color:#F0EEFF;display:flex;align-items:center;gap:6px}}
.user-name .fire{{font-size:18px}}
.status{{display:inline-flex;align-items:center;gap:4px;padding:4px 12px;border-radius:16px;font-size:11px;font-weight:700;}}
.status.active{{background:rgba(255,80,20,0.12);color:#FF8C00;border:1px solid rgba(255,80,20,0.12);}}
.status.inactive{{background:rgba(239,68,68,0.12);color:#F87171;border:1px solid rgba(239,68,68,0.12);}}
.uuid-box{{background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.04);border-radius:8px;padding:6px 10px;font-size:9px;font-family:monospace;color:#8A4A3A;word-break:break-all;margin:4px 0 12px;cursor:pointer}}
.uuid-box:hover{{background:rgba(255,80,20,0.05)}}
.info-grid{{display:grid;gap:8px;margin:12px 0}}
.info-item{{background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.03);border-radius:10px;padding:10px 14px;display:flex;justify-content:space-between;align-items:center}}
.info-label{{font-size:11px;color:#8A4A3A;display:flex;align-items:center;gap:5px}}
.info-label i{{font-size:13px;color:#FF6B35}}
.info-value{{font-size:13px;font-weight:700;color:#F0EEFF}}
.info-value.used{{color:#FF8C00}}
.info-value.proto{{font-size:10px;background:rgba(255,80,20,0.06);padding:2px 10px;border-radius:6px;border:1px solid rgba(255,80,20,0.06);}}
.progress{{margin:14px 0 16px}}
.progress-bar{{height:5px;border-radius:4px;background:rgba(255,255,255,0.04);overflow:hidden}}
.progress-fill{{height:100%;border-radius:4px;background:linear-gradient(90deg,#FF6B35,#FF4500,#FF8C00);width:{percent:.1f}%;transition:width 1s ease}}
.progress-text{{display:flex;justify-content:space-between;font-size:10px;color:#8A4A3A;margin-top:4px}}
.progress-text .pct{{font-weight:700;color:#F0EEFF}}
.vless-section{{background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.04);border-radius:10px;padding:12px 14px;margin:12px 0}}
.vless-label{{font-size:9px;color:#8A4A3A;font-weight:700;text-transform:uppercase;letter-spacing:.05em;display:flex;align-items:center;gap:5px;margin-bottom:6px}}
.vless-label i{{color:#FF6B35;font-size:12px}}
.vless-link{{font-family:monospace;font-size:9px;color:#FF8C00;word-break:break-all;line-height:1.6;background:rgba(0,0,0,0.3);padding:8px 10px;border-radius:6px;border:1px solid rgba(255,80,20,0.04);}}
.actions{{display:flex;gap:6px;margin-top:12px;flex-wrap:wrap}}
.btn{{font-family:inherit;font-size:11px;font-weight:600;border-radius:8px;padding:7px 14px;cursor:pointer;display:inline-flex;align-items:center;gap:5px;border:none;transition:all .2s;white-space:nowrap;flex:1;justify-content:center}}
.btn i{{font-size:13px}}
.btn-primary{{background:linear-gradient(135deg,#FF6B35,#FF4500);color:#fff;box-shadow:0 4px 20px rgba(255,80,20,0.2)}}
.btn-primary:hover{{transform:translateY(-2px);box-shadow:0 8px 30px rgba(255,80,20,0.3)}}
.btn-secondary{{background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);color:#8A4A3A}}
.btn-secondary:hover{{background:rgba(255,255,255,0.06);color:#F0EEFF}}
.btn-success{{background:rgba(16,185,129,0.08);border:1px solid rgba(16,185,129,0.1);color:#34D399}}
.btn-success:hover{{background:rgba(16,185,129,0.12)}}
.footer{{margin-top:18px;padding-top:14px;border-top:1px solid rgba(255,255,255,0.02);text-align:center;font-size:9px;color:#5A3A2A}}
.footer .eagle{{color:#FF6B35;font-weight:700}}
.toast{{position:fixed;bottom:20px;left:50%;transform:translateX(-50%) translateY(50px);background:rgba(15,15,30,0.9);backdrop-filter:blur(20px);border:1px solid rgba(255,80,20,0.1);color:#F0EEFF;border-radius:10px;padding:10px 20px;font-size:12px;opacity:0;transition:all .3s;z-index:999;pointer-events:none;display:flex;align-items:center;gap:6px;box-shadow:0 10px 40px rgba(0,0,0,0.4)}}
.toast.show{{opacity:1;transform:translateX(-50%) translateY(0)}}
.toast.ok{{border-color:rgba(16,185,129,0.2);color:#34D399}}
@media(max-width:460px){{.card{{padding:20px 16px 18px}}.user-name{{font-size:17px}}.brand-icon{{width:36px;height:36px;font-size:18px}}.info-item{{padding:8px 10px}}.btn{{font-size:10px;padding:5px 10px}}}}
</style>
</head>
<body>
<div class="fire-particles">
    <div class="fire-particle" style="left:5%;animation-delay:0s;width:6px;height:6px"></div>
    <div class="fire-particle" style="left:25%;animation-delay:3s;width:8px;height:8px"></div>
    <div class="fire-particle" style="left:45%;animation-delay:5s;width:5px;height:5px"></div>
    <div class="fire-particle" style="left:65%;animation-delay:2s;width:7px;height:7px"></div>
    <div class="fire-particle" style="left:85%;animation-delay:6s;width:6px;height:6px"></div>
</div>
...
<div class="card">
    ...
    <div class="info-grid">
        ...
        <div class="info-item"><span class="info-label"><i class="ti ti-clock"></i> آخرین اتصال</span><span class="info-value" style="font-size:11px;">{last_connected_text}</span></div>
